...
let prefixes xs = match xs with 
  [] -> [[]]
  | h::t -> [[]]

let addFront a lxs = List.fold_right(fun xs alxs -> (a::xs)::alxs) lxs [];;

let prefixes xs = List.fold_right(fun x prefixList -> [[]]@(
  addFront x prefixList)) xs [[]]

let inject a xs = List.fold_right2(fun pre suf injected -> 
  (pre@([a]@suf))::injected) (prefixes xs) (suffixes xs) [];;
